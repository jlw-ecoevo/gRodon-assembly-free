...
statsMeta <- function(simname,bg="all"){
  ...
  high_score <- readLines(paste0(simname,"_fraggenescan.highscore"))
  genes <- genes[names(genes) %in% high_score]
  highly_expressed <- names(genes) %in% ribo_names
  print(paste("nHE:",length(highly_expressed)))
...
save(sim_df, file = "CodonStatistics_simmeta150_95pid_score125.rda")